...
class JiraDataBase:
    ...
    def __init__(self, search_query, columns, output_file_name):
    ...
    def set_column_value(self, mapped_column_for_status, created):
        if mapped_column_for_status == '' or mapped_column_for_status is None:
            print(
                f'mapping not found for Issue ID: {self.csv_row_list[JiraDataBase._idColumnName]} stauts changed on {created}')
            return
